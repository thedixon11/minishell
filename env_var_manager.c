...
char  *env_var_checker(t_env *env, char *var_env)
{
  ...
  //NOTE: ici, on va comparer notre var_env et voir si elle existe dans l'environnement
...
char  *extract_env_var(char *str, int *i)
{
  ...
  //NOTE: une fois la fin de la var_env trouvee, on va l'extraire de la str
  //grace a substr. on a besoin pour ca de la len de la var_env
  len = end - *i;
  var_env = ft_substr(str, *i, len);

  //NOTE: on place i juste apres la env var
  *i = end;
  return (var_env);
}
...
{
  //NOTE: i peut soit etre i_value, soit i_quote en depend de la situation
  // str peut soit etre value, soit inside_quote en depend de la situation

  char  *xpansion_result;
  char  *var_env;

  //NOTE: pour commencer, il faut qu'on extraie la var env potentielle
  //contenue dans la string
  var_env = extract_env_var(str, i);

  //NOTE: une fois la env_var extraite, on va checker si elle existe bien
