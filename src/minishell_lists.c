...
t_line	*build_line_list(void)
{
	...
	head = append_line(head, new_line_node(T_COMMAND,      "wc -l",   1));
	head = append_line(head, new_line_node(T_OUTPUT_TRUNC, "outfile1",1));
	return (head);
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : < infile1 cat | wc -l > outfile1 ===\n\n");
	...
	execution(data, line_list, env_list);
