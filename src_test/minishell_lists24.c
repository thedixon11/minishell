...
**   echo -nnnn -na salut
**
**   "echo -nnnn -na salut" → T_COMMAND (builtin + args, one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   echo is a builtin. Flag parsing rules for minishell's echo:
**
**     -nnnn → valid flag, any number of consecutive 'n' after '-'
**             counts as -n → suppress trailing newline
**     -na   → INVALID flag — 'a' is not 'n', so "-na" is NOT treated
**             as a flag but as a literal argument to print.
**             bash behaviour: once a non-valid flag is encountered,
**             everything from that point is treated as an argument,
**             including "-na" itself.
**
**   So the output will be:
**     "-na salut\n"   ← -nnnn suppresses nothing wait...
**
**   Actually correct bash behaviour:
**     -nnnn → valid, no newline
**     -na   → invalid flag → treated as literal string argument
**   Output: "-na salut" (no trailing newline because -nnnn was valid)
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "echo -nnnn -na salut", 0));
	return (head);
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : echo -nnnn -na salut ===\n\n");
