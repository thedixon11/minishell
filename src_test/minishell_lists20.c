...
**
**   AA=salut AA AB=capibara AB=
**
**   "AA=salut AA AB=capibara AB=" → T_COMMAND (one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   NOTE: this is a bare assignment without any command — in bash,
**   this sets the variables in the current shell environment only,
**   without exporting them. Your executor needs to detect that the
**   first token is a key=value pair with no preceding command name
**   and handle it as a shell variable assignment, NOT as a command.
**
**   Processing left to right:
**
**     AA=salut   → set AA to "salut"
**     AA         → no '=' → mark AA, do NOT touch its value → AA stays "salut"
**     AB=capibara→ set AB to "capibara"
**     AB=        → '=' with empty value → set AB to "" (overwrites "capibara")
**
**   Final state:
**     AA → "salut"
**     AB → ""
...
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "AA=salut AA AB=capibara AB=", 0));
	return (head);
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : AA=salut AA AB=capibara AB= ===\n\n");
