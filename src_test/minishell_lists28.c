...
**   cd ..
**
**   "cd .." → T_COMMAND (builtin + arg, one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   Relative path — goes one directory up from current PWD.
**   No tilde, no variable expansion needed.
**   Your cd builtin calls directly:
**     chdir("..")
**   then updates OLDPWD and PWD in t_env:
**     OLDPWD → current PWD value
**     PWD    → result of getcwd() after chdir()
**
**   Edge case: if already at root "/" → chdir("..") stays at "/"
**   silently, no error. getcwd() will return "/" and PWD is updated
**   to "/" accordingly.
*/
