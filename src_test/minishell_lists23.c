...
**
**   export 123=salut AA=salut AB AC= AA=BONJOUR
**
**   "export 123=salut AA=salut AB AC= AA=BONJOUR" → T_COMMAND (one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   Your export builtin processes left to right:
**
**     123=salut  → INVALID — identifier starts with a digit, not a letter
**                  or '_'. Print error and set exit code to 1, but
**                  CONTINUE processing the remaining arguments.
**                  bash: "export: `123=salut': not a valid identifier"
**     AA=salut   → set AA to "salut" and export it
**     AB         → no '=' → mark AB for export, content=NULL if new
**     AC=        → set AC to "" (empty string) and export it
**     AA=BONJOUR → update AA to "BONJOUR" (overwrites "salut")
**
**   Final state:
**     AA → "BONJOUR"  (set twice, last write wins)
**     AB → NULL       (marked for export, no value assigned)
**     AC → ""         (exported with empty value)
**
**   Exit code: 1 (because of the invalid identifier 123=salut),
**   even though the other args were processed successfully.
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "export 123=salut AA=salut AB AC= AA=BONJOUR", 0));
	return (head);
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : export 123=salut AA=salut AB AC= AA=BONJOUR ===\n\n");
