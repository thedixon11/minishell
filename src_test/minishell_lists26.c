...
**
**   "cd ~/42cursus/minishell/" → T_COMMAND (builtin + arg, one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   '~' at the start of an unquoted token is a tilde expansion —
**   your expander must replace it with the value of HOME in t_env
**   BEFORE cd processes the path:
**
**     e.g. HOME="/Users/me" → expands to "/Users/me/42cursus/minishell/"
**
**   Tilde expansion rules:
**     ~/path  → $HOME/path       (only if '~' is unquoted and at start)
**     "~/path" → literal "~/path" (double quotes suppress tilde expansion)
**     '~/path' → literal "~/path" (single quotes suppress tilde expansion)
**
**   After expansion your cd builtin calls:
**     chdir("/Users/me/42cursus/minishell/")
**   then updates OLDPWD and PWD in t_env accordingly.
**
**   If the path doesn't exist → print error, exit code 1.
**   If too many args (e.g. "cd a b") → print error, exit code 1.
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "cd ~/42cursus/minishell/", 0));
	return (head);
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : cd ~/42cursus/minishell/ ===\n\n");
