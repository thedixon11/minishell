...
**
**   < farmer /bin/cat > lesang
**
**   "farmer"   → T_INPUT        (type encodes "<")
**   "/bin/cat" → T_COMMAND      (no arguments)
**   "lesang"   → T_OUTPUT_TRUNC (type encodes ">")
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   "/bin/cat" is an absolute path — your executor must NOT search
**   PATH at all. It calls execve directly with the path as-is:
**     execve("/bin/cat", ...)
**   If the file doesn't exist or has no execute permission:
**     bash: /bin/cat: No such file or directory → exit code 127
**     bash: /bin/cat: Permission denied          → exit code 126
**
**   This is the key distinction from plain "cat":
**     "cat"      → search PATH, find and execve
**     "/bin/cat" → execve directly, no PATH search needed
**
**   Your executor should check: does content start with '/' or "./"?
**   → yes → absolute/relative path → execve directly
**   → no  → bare name → walk PATH
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_INPUT,        "farmer",   0));
	head = append_line(head, new_line_node(T_COMMAND,      "/bin/cat", 0));
	head = append_line(head, new_line_node(T_OUTPUT_TRUNC, "lesang",   0));
	return (head);
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : < farmer /bin/cat > lesang ===\n\n");
