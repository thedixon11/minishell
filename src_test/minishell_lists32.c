...
**
**   exit capibara 123 456
**
**   "exit capibara 123 456" → T_COMMAND (builtin + args, one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   Two problems hit in order — bash checks numeric validity FIRST,
**   then checks argument count:
**
**   bash actual behaviour:
**     "capibara" is not a valid number → numeric argument required
**     → print error to stderr, exit immediately with code 2:
**       bash: exit: capibara: numeric argument required
**     → shell DOES exit (unlike the too-many-args case)
**
**   This is the opposite of the too-many-args case ("exit 1 2"):
**     too many args + all numeric   → error, do NOT exit, code 1
**     first arg non-numeric         → error, DO exit, code 2
**
**   So your exit builtin priority must be:
**     1. Check if first arg is a valid number
**        → if not → print error, exit(2)
**     2. Check if there is more than one arg
**        → if so  → print error, do NOT exit, return 1
**     3. Otherwise → exit(code % 256)
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "exit capibara 123 456", 0));
	return (head);
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : exit capibara 123 456 ===\n\n");
