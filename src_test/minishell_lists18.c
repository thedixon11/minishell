...
**
**   export AA AA= AA=salut AB AC=
**
**   "export AA AA= AA=salut AB AC=" → T_COMMAND (builtin + args, one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   Your export builtin must handle each argument differently:
**
**     AA       → no '=' → mark AA for export, do NOT touch its value
**                          if AA doesn't exist yet, create it with content=NULL
**     AA=      → '=' with empty value → set AA to ""  (empty string)
**     AA=salut → '=' with value → set AA to "salut"
**     AB       → no '=' → mark AB for export, content=NULL if new
**     AC=      → '=' with empty value → set AC to ""  (empty string)
**
**   Processing is left to right — AA= then AA=salut both hit AA,
**   so the final value of AA in t_env will be "salut" (last write wins).
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "export AA AA= AA=salut AB AC=", 0));
	return (head);
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : export AA AA= AA=salut AB AC= ===\n\n");
