...
**   ls -la
**
**   "ls -la" → T_COMMAND (command + flag, one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   "ls" is a bare command name — executor must walk PATH to find it.
**   No redirections → stdin/stdout stay untouched.
**   exit code: 0 on success, 1 if a path argument doesn't exist.
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "ls -la", 0));
	return (head);
}
