...
**   export
**
**   "export" → T_COMMAND (builtin, no arguments)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   NOTE: export with no arguments is a builtin that prints all
**   exported env variables — it must NOT be executed via execve
**   but handled directly in the parent process so it can
**   access and print your t_env list.
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "export", 0));
	return (head);
}
