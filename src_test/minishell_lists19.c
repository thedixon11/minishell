...
**   export AA=salut AA
**
**   "export AA=salut AA" → T_COMMAND (builtin + args, one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   Your export builtin processes left to right:
**
**     AA=salut → set AA to "salut" in t_env
**     AA       → no '=' → mark AA for export, do NOT touch its value
**
**   Final state of AA: content="salut" (the second arg has no '='
**   so it never overwrites the value set by the first).
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "export AA=salut AA", 0));
	return (head);
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : export AA=salut AA ===\n\n");
