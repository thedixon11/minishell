...
**   exit 123 445 kasdo
**
**   "exit 123 445 kasdo" → T_COMMAND (builtin + args, one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   bash checks numeric validity of the FIRST arg first, then arg count:
**
**     "123" → valid number → numeric check passes
**     but there are 3 args → too many arguments
**     → print error to stderr, do NOT exit, return 1:
**       bash: exit: too many arguments
**
**   "445" and "kasdo" are never even looked at — bash stops after
**   detecting too many args once the first is confirmed numeric.
**
**   Full exit decision tree reminder:
**     1. first arg non-numeric → print error, exit(2)     ← prev case
**     2. more than one arg     → print error, return 1,   ← THIS case
**                                DO NOT exit
**     3. one valid numeric arg → exit(code % 256)
**     4. no args               → exit(last_exit_code)
**
**   exit code: 1 (error returned, shell keeps running)
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "exit 123 445 kasdo", 0));
	return (head);
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : exit 123 445 kasdo ===\n\n");
