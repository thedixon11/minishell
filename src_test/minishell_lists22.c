...
**   export $CAP
**
**   "export $CAP" → T_COMMAND (builtin + arg, one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   The expander runs BEFORE the export builtin processes its arguments.
**   So $CAP is expanded first against t_env:
**
**     e.g. if CAP="SALUT EASY" → after expansion the command becomes:
**          export SALUT EASY    ← two separate args due to word splitting
**          → export tries to export "SALUT" and "EASY" as variable names
**          → both are invalid identifiers (no '=', and bash would
**            attempt to mark them for export if they exist in env)
**
**     e.g. if CAP="MYVAR=hello" → after expansion:
**          export MYVAR=hello   ← one arg, valid assignment
**          → sets and exports MYVAR to "hello"
**
**     e.g. if CAP is unset → expands to "" → export with empty arg
**          → bash silently ignores it or errors depending on context
**
**   NOTE: this is the classic expand-then-execute order.
**   content stores the raw string — expander fills content_xpand,
**   and the export builtin works off content_xpand, not content.
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "export $CAP", 0));
	return (head);
}
