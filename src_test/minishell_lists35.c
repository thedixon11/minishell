...
**
**   "cat" is a relative command name — your executor must search
**   for it by walking PATH in t_env and trying each directory:
**     /usr/local/bin/cat → exists? → execve it
**     /usr/bin/cat       → exists? → execve it
**     /bin/cat           → exists? → execve it
**     ...
**   If not found in any PATH directory → exit code 127:
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : < farmer cat > lesang ===\n\n");
