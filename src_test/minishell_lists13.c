...
**   The expander walks it left to right:
**
**     "echo" → literal command name
**     "$?"   → unquoted → special parameter, expander MUST replace it
**              with the exit status of the last executed command (0-255)
**              stored in your t_data or equivalent.
**
**   NOTE: $? is NOT a regular env var — it cannot be found in t_env.
**   Your expander needs a dedicated branch to handle it separately
**   from standard $VAR expansions.
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "echo $?", 0));
	return (head);
}
