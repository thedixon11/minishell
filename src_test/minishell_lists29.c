...
**
**   unset HOME PWD LESANG OLDPWD
**
**   "unset HOME PWD LESANG OLDPWD" → T_COMMAND (builtin + args, one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   unset is a builtin that MUST run in the parent process — if forked,
**   removals from t_env die with the child and the parent never sees them.
**
**   Your unset builtin processes each arg left to right and removes
**   the matching node from t_env:
**
**     HOME   → found → unlink and free node from t_env
**     PWD    → found → unlink and free node from t_env
**     LESANG → not found → silently ignore, no error, exit code stays 0
**     OLDPWD → found → unlink and free node from t_env
**
**   Rules:
**     - If the variable doesn't exist → silently ignore (no error)
**     - Invalid identifier (e.g. "unset 123") → print error, exit code 1,
**       but continue processing remaining args
**     - Exit code 0 if all args are valid identifiers (even if not found)
**
**   NOTE: unsetting HOME and OLDPWD will directly affect subsequent
**   cd calls — a good reason to test this before testing cd.
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "unset HOME PWD LESANG OLDPWD", 0));
	return (head);
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : unset HOME PWD LESANG OLDPWD ===\n\n");
