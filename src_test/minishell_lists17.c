...
**
**   export AA=salut AB=lesang
**
**   "export AA=salut AB=lesang" → T_COMMAND (builtin + arguments, one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   NOTE: export with arguments is still a builtin — must run in the
**   parent process. Your executor parses the content string to extract
**   each key=value pair and adds/updates them in your t_env list:
**
**     AA=salut  → new t_env node: name="AA"  content="salut"
**     AB=lesang → new t_env node: name="AB"  content="lesang"
**
**   If AA or AB already exist in t_env, their content is updated.
**   If the key has no '=' (e.g. "export AA"), the var is marked for
**   export but its value stays unchanged.
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "export AA=salut AB=lesang", 0));
	return (head);
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : export AA=salut AB=lesang ===\n\n");
