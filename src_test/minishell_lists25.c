...
**   cd
**
**   "cd" → T_COMMAND (builtin, no arguments)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   cd with no arguments behaves like "cd $HOME" — changes the current
**   working directory to the value of HOME in t_env.
**
**   NOTE: cd is a builtin that MUST run in the parent process.
**   If forked, the chdir() call affects only the child — the parent's
**   working directory never changes, which breaks every subsequent
**   relative path resolution.
**
**   Your cd builtin should also update PWD and OLDPWD in t_env:
**     OLDPWD → current value of PWD before the cd
**     PWD    → new working directory after chdir()
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "cd", 0));
	return (head);
}
