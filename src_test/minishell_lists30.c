...
**   pwd le sang tigre
**
**   "pwd le sang tigre" → T_COMMAND (builtin + args, one node)
**
**   All cmd_nb 0 — single command, no pipes, no redirections.
**
**   pwd is a builtin that prints the current working directory.
**   "le sang tigre" are extra arguments — bash silently ignores them.
**   Your pwd builtin should do the same:
**
**     1. Ignore all arguments entirely
**     2. Print the value of PWD from t_env (or getcwd() if PWD is unset)
**     3. Always exit code 0 unless getcwd() itself fails
**
**   NOTE: pwd does NOT need to run in the parent process — it only
**   reads state, it never modifies the shell environment. Forking is
**   fine here, unlike cd, export, unset, and exit.
*/
t_line	*build_line_list(void)
{
	...
	head = NULL;
	head = append_line(head, new_line_node(T_COMMAND, "pwd le sang tigre", 0));
	return (head);
}
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* ── command list ───────────────────────────────────────────────────── */
	printf("\n=== COMMAND LIST : pwd le sang tigre ===\n\n");
